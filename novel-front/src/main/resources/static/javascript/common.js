    ...
    $.ajax({
        ...
        success: function(data){
            ...
            if(data.code == 200){
                ...
                var pathnameuse=window.location.pathname;
                if(pathnameuse.split("/")[1]=="book"){
                    console.log("18");//能显示
                    var bookId=document.getElementById("bookId").value;
                    var pageId=document.getElementById("preContentId").value;
                    if("/book/"+bookId+"/"+pageId+".html" == window.location.pathname){
                        $('#canGetContent').html('                    <button id="canGetContent" type="button" style="float: right;color: #4c88bb;background-color: #ebe5d8;border: none;" onclick="getuText()">!从本章内容开始创建章节!</button>');
                ...
                userNameForComment=data.data.nickName;
