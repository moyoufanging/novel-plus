...
app.controller('myCtrl', ['$scope', '$compile','httpService', function($scope,$compile, httpService) {
	var mockusers = [{
		"id" : "1",
		"username" : "游客",
    ...
    $scope.setcommentUser=function(){
             window.localStorage.setItem("id", mockusers[0].id); //也就是1,主要是为了避免格式的冲突
        //alert("him");
...
//      alert("12");
      $.ajax({
              ...
              success: function (returndata) {
                  if(returndata!=""&&returndata!=''&&returndata.data!=null){
                      //document.getElementById('shuyouquanid').innerHTML=returndata.data.replace("<p>||此处插入评论框...||</p>",'<i name="commentNum" datasegid="1"  ng-click="showSameHobbyCWL();setcommentUser();" onclick="showCommentDiv(this)" style="color:#B5B5B5;" class="layui-icon layui-icon-reply-fill"></i>');
                      uw=returndata.data.replace(/\|\|此处插入评论框...\w+\|\|/g, function(word){
                        num1 = parseInt(word.replace(/[^\d]/g,' '));
                        var html = '<span name="commentNum" datasegid="'+num1+'"  ></span>';
                        //用$compile进行编译
                        //添加到页面中，或者你任何想添加的位置。});
                        return html;
                      ...
                      document.getElementById('shuyouquanid').innerHTML=uw;


                        //用$compile进行编译
                        //添加到页面中，或者你任何想添加的位置。});
                      var xe=document.getElementsByName("commentNum");
                      for (let i=0; i<xe.length; i++){
                         var html = '<i id="commentNum"  datasegid="'+xe[i].getAttribute('datasegid')+'"  ng-click="setcommentUser();showSameHobbyCWL();" onclick="showCommentDiv(this)" style="color:#B5B5B5;" class="layui-icon layui-icon-reply-fill"></i>';
                         var $html2 = $compile(html)($scope);
//                         console.log("id"+xe[i].getAttribute('datasegid'));
                         angular.element(xe[i]).append($html2);
                           //angular.element(xe[i]).append($html);
                      }
    ...
    $scope.showSameHobbyCWL= function(){
        //alert("showSameHobbyCWL2");
        var bookId1=document.getElementById("bookId").value;
        //alert("尝试出内容"); //有内容显示
        var lineId1=document.getElementById("nowLineIdHidden").value;
...
//                var commentThumsJson=data.flcs;
    ...
    $scope.firstCommentbefore=function(sayingId){
        ...
        var lineId=document.getElementById("nowLineIdHidden").value;
        $scope.firstComment(sayingId,bookId,pageId,lineId);
    }
    //这个是书友圈的评论功能
    $scope.firstCommentbeforeShuyou2=function(sayingId){
        console.log();
    }
