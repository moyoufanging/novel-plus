...
public class PageController extends BaseController {
    ...
    @Autowired
    private final FrontBookMapper frontBookMapper;
    ...
    @SneakyThrows
    @RequestMapping("/author_shuyouquan_editor/{bookId}.html")
    public String  authorShuyouquanEditor(@PathVariable("bookId") Long bookId,Model model, HttpServletRequest request){
        //访问作者专区
        UserDetails user = getUserDetails(request);
        if (user == null) {
            //未登录
        ...
        //检测是是否是15的作者
        if(frontBookMapper.getBookAuthorId(bookId)==frontBookMapper.getUserAuthorId(user.getId())){
            return "author/author_shuyouquan_editor";


        }else{
            return "404";
        }
        //author/author_shuyouquan_editor.html
    }
    ...
    public String bookContent(@PathVariable("bookId") Long bookId, @PathVariable("bookIndexId") Long bookIndexId, HttpServletRequest request, Model model) {
        ...
        //加载小说下一掌节信息线程，该线程在加载小说章节信息线程执行完毕后才执行
        ...
        CompletableFuture<BookContent> bookContentCompletableFuture = bookIndexCompletableFuture.thenApplyAsync((bookIndex) -> {
            ...
            //替换修改,即将用来显示的小说内容
            String contentStr=bookContent.getContent()+"<br>";
            //contentStr.replace("<br>", "<i name=\"commentNum\" datasegid=\"1\" class=\"layui-icon layui-icon-reply-fill\">12</i>\n<br>");
            bookContent.setContent(contentStr.replace("<br>", "<i name=\"commentNum\" datasegid=\"1\"  ng-click=\"showCommentWithLine();setcommentUser()\" onclick=\"showCommentDiv(this)\" style=\"color:#B5B5B5;\" class=\"layui-icon layui-icon-reply-fill\">0</i>\n<br>"));
